...
# 随机的User-Agent下载中间件类RandomUserAgent
...
# 设置代理,使用代理ip与端口号去下载的下载中间件类RandomProxy
class RandomProxy(object):
    def process_request(self,request,spider):
        # 取出的proxy是个字典dict,有键ip_port与user_pwd
        proxy = random.choice(PROXIES)
        if proxy.has_key('user_pwd'):
            # 有用户名与密码时
            # 1.对用户名和密码进行base64编码转换
